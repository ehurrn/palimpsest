...
def mask_context_text(
    ...
    for ent in entities:
        approved = ent["living_status"] == "deceased_historical" and _is_approved(
            ent["entity_id"], conn, approved_ids
        )
...
def get_citation(row: Dict[str, Any], prefix: str) -> Dict[str, Any]:
    ...
    return {
        ...
        or f"https://www.osti.gov/opennet/servlets/purl/{doc_id}.pdf",
...
@mcp.tool()
def palimpsest_find_redaction_gaps(
    min_score: float = 0.65,
    status: str = "candidate",
    kind: str | None = None,
    limit: int = 20,
    min_tier: str = "surfaceable",
) -> str:
    """List redaction gaps with scores above the threshold and masked person entities."""
    logger.info(
        f"Tool call: palimpsest_find_redaction_gaps(min_score={min_score}, status={status!r}, kind={kind!r}, limit={limit}, min_tier={min_tier!r})"
    )
    ...
    try:
        ...
        page_text = fetch_pages_text(conn, {(row["e_doc_id"], row["e_page_no"]) for row in rows})

        from palimpsest.eval.gate import apply_gate, load_artifact

        _artifact = load_artifact(cfg)
        _enforcement = (getattr(cfg, "eval", {}) or {}).get("gate_enforcement", "enforce")
        # Fail open when there is no calibration artifact: without one every row is
        # "tentative", so enforcing a "surfaceable" floor would drop everything.
        # Surface the (still person-masked) rows annotated as tentative instead.
        ...
        for row in rows:
            ...
            masked_p_text = get_masked_text_for_page(
                row["e_doc_id"], row["e_page_no"], p_text, conn, approved_ids
            )
            ...
            r_ctx_before = mask_context_text(
                row["r_doc_id"], row["r_page_no"], row["context_before"], conn, approved_ids
            )
            r_ctx_after = mask_context_text(
                row["r_doc_id"], row["r_page_no"], row["context_after"], conn, approved_ids
            )
            ...
            results.append(
                {
                    ...
                    "requires_review": row["e_kind"] == "person"
                    and masked_text.startswith("PERSON-"),
                }
            )
...
def palimpsest_search(query: str, limit: int = 10) -> str:
    ...
    try:
        ...
        page_text = fetch_pages_text(conn, {(row["doc_id"], row["page_no"]) for row in rows})
        ...
        for idx, cid in enumerate(indices[0]):
            ...
            masked_p_text = get_masked_text_for_page(
                row["doc_id"], row["page_no"], p_text, conn, approved_ids
            )

            c_start = row["char_start"]
            c_end = row["char_end"]
            masked_chunk_text = (
                masked_p_text[c_start:c_end]
                if (c_start is not None and c_end is not None)
                else row["text"]
            )
            ...
            results.append(
                {
                    ...
                    "citation": {
                        ...
                        or f"https://www.osti.gov/opennet/servlets/purl/{row['doc_id']}.pdf",
                        "title": row["title"],
...
def palimpsest_get_document(doc_id: str, page_no: int | None = None) -> str:
    ...
    try:
        # Fetch metadata
        cur = conn.execute(
            "SELECT doc_id, title, accession, source_url, status, page_count FROM documents WHERE doc_id = ?",
            (doc_id,),
        )
        doc = cur.fetchone()
        if not doc:
        ...
        if page_no is not None:
            cur_p = conn.execute(
                "SELECT page_no, text FROM pages WHERE doc_id = ? AND page_no = ?",
                (doc_id, page_no),
            )
        else:
            cur_p = conn.execute(
                "SELECT page_no, text FROM pages WHERE doc_id = ? ORDER BY page_no ASC", (doc_id,)
            )
        ...
        for p_row in pages_rows:
            ...
            pages.append({"page_no": p_num, "text": masked_text})

        # Fetch redactions
        cur_r = conn.execute(
            "SELECT redaction_id, page_no, kind, label, x0, y0, x1, y1, context_before, context_after FROM redactions WHERE doc_id = ?",
            (doc_id,),
        )
        ...
        for r_row in red_rows:
            p_num = r_row["page_no"]
            r_ctx_before = mask_context_text(
                doc_id, p_num, r_row["context_before"], conn, approved_ids
            )
            r_ctx_after = mask_context_text(
                doc_id, p_num, r_row["context_after"], conn, approved_ids
            )
            redactions.append(
                {
        ...
        # Fetch entities
        cur_e = conn.execute(
            "SELECT entity_id, page_no, kind, text, norm, char_start, char_end, x0, y0, x1, y1, living_status FROM entities WHERE doc_id = ?",
            (doc_id,),
        )
        ...
        return json.dumps(
            {"metadata": doc_meta, "pages": pages, "redactions": redactions, "entities": entities},
            indent=2,
        )
    except Exception as e:
        conn.close()
