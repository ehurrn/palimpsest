"""Ollama-powered features extraction worker for palimpsest.

Mirrors gemini_features_worker.py but calls a local Ollama instance instead
of the agy/Gemini API. Designed for M4 running qwen3:8b.

Thinking mode is disabled via /no_think in the system prompt so responses
are direct JSON without reasoning preamble.

Usage:
    uv run python scripts/ollama_features_worker.py [--concurrency 2] [--batch-size 5]
"""
...
MODEL = "qwen3:8b"
WORKER_ID = "ollama-features"

MAX_CHARS_PER_PAGE = 4000
# Local model has smaller context than Gemini — keep batches tight
MAX_PROMPT_CHARS = 24_000
_MAX_PAGES_PER_DOC = (MAX_PROMPT_CHARS - 2000) // (MAX_CHARS_PER_PAGE + 30)

# ---------------------------------------------------------------------------
# Prompt — identical schema to gemini_features_worker; /no_think disables
# Qwen3 reasoning preamble so we get raw JSON directly.
# ---------------------------------------------------------------------------
...
def extract_json(text: str) -> dict:
    ...
    # Strip <think>...</think> block if model ignored /no_think
...
def process_batch(
    ...
    for sub in sub_batches:
        ...
        print(f"{tag}Ollama call — {len(sub)} docs, {len(prompt):,} chars")
        try:
            raw = call_ollama(prompt)
            data = extract_json(raw)
        except Exception as exc:
            print(f"{tag}Ollama error: {exc}")
            for doc_id, _ in sub:
    ...
    try:
        http.post(
            ...
            json={
                "job_id": job["job_id"],
                "worker_id": WORKER_ID,
                "error": "ollama extraction failed",
                "retryable": True,
            },
            timeout=10,
        )
...
def main() -> None:
    ...
    parser.add_argument("--concurrency", type=int, default=2, help="Parallel Ollama threads (default 2)")
    parser.add_argument("--batch-size", type=int, default=5, help="Docs per Ollama call (default 5)")
    ...
    print(
        f"Ollama features worker — broker {broker_url} | model {MODEL} | "
